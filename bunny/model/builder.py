...
def load_pretrained_model(model_path, model_base, model_name, model_type,
	...
	if 'lora' in model_name.lower() and model_base is not None:
		...
		if model_type == 'phi-1.5' or model_type == 'phi-2':
			...
			model = BunnyPhiForCausalLM.from_pretrained(model_base,
		...
		if model.lm_head.weight.shape[0] != token_num:
			model.lm_head.weight = torch.nn.Parameter(
				torch.empty(token_num, tokem_dim, device=model.device,
							dtype=model.dtype))
		...
		if model_type == 'phi-1.5' or model_type == 'phi-2':
			...
			model = BunnyPhiForCausalLM.from_pretrained(model_path,
														low_cpu_mem_usage=True,
														**kwargs)
		elif model_type == 'qwen-2' or model_type == 'qwen-1.5':
			# TODO check qwen padding and special tokens
	...
	# if model_type == 'qwen-2' or model_type == 'qwen-1.5':
