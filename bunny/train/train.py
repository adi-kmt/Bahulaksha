...
def get_peft_state_maybe_zero_3(named_params, bias):
	...
	to_return = {k: maybe_zero_3(v, ignore_status=True) for k, v in
				 to_return.items()}
	return to_return
...
def get_peft_state_non_lora_maybe_zero_3(named_params, require_grad_only=True):
	...
	to_return = {k: maybe_zero_3(v, ignore_status=True).cpu() for k, v in
				 to_return.items()}
	return to_return
...
def train():
	...
	elif model_args.model_type == 'qwen-2' or model_args.model_type == 'qwen-1.5':
	...
	if training_args.bits in [4, 8]:
		...
		model.config.torch_dtype = (
			torch.float32 if training_args.fp16 else (
				torch.bfloat16 if training_args.bf16 else torch.float32))
		''' TODO Some reason Qwen doesn't work well with quantisation 
		https://github.com/OpenAccess-AI-Collective/axolotl/blob/main/src/axolotl/utils/models.py#L683
		'''
